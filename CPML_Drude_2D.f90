...
do i=1,Nx-1
 if(i<=npml)then
  ...
  C1_ey(i) = (eps_r*eps0/dt-0.5*sige_x(i)-0.5*A2)/(eps_r*eps0/dt+0.5*sige_x(i)+0.5*A2)
  C2_ey(i) = 1.0/(eps_r*eps0/dt+0.5*sige_x(i)+0.5*A2)
  C3_ey(i) = 0.5*(A1+1.0)/(eps_r*eps0/dt+0.5*sige_x(i)+0.5*A2)
 elseif (i>=(Nx+1-npml))then
  den_ex(i)=1.0/(kappae_x(ii)*dx)
  C1_ey(i) = (eps_r*eps0/dt-0.5*sige_x(ii)-0.5*A2)/(eps_r*eps0/dt+0.5*sige_x(ii)+0.5*A2)
  C2_ey(i) = 1.0/(eps_r*eps0/dt+0.5*sige_x(ii)+0.5*A2)
  C3_ey(i) = 0.5*(A1+1.0)/(eps_r*eps0/dt+0.5*sige_x(ii)+0.5*A2)
  ii=ii-1
 else
  den_ex(i)=1.0/dx
