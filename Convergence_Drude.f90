...
double precision, parameter :: x_drude1 = -8E-9, x_drude2 = 8E-9, y_drude1 = -18E-9, y_drude2 = -2E-9
...
function Drude_CPML() result(H_sum)

   double precision :: H_sum
...
double precision :: A1, A2, Ca, Cb, Cc
...
logical GR
 ...
 character(len = 3), parameter :: str_Drude_x = 'FBx', str_Drude_y = 'FBy'
...
A1=(2.0-GammaD*dt)/(2.0+GammaD*dt)
A2=eps0*omegaD*omegaD*dt/(2.0+GammaD*dt)
 Ca=(eps_r*eps0/dt-0.5*A2)/(eps_r*eps0/dt+0.5*A2)
 Cb=1.0/(eps_r*eps0/dt+0.5*A2)
 Cc=0.5*(A1+1.0)/(eps_r*eps0/dt+0.5*A2)
...
do i=1,Nx-1
 do j=1,N_loc
  if( &
    (y(j)>=y_drude1).and.(y(j)<=y_drude2).and.(xM2(i)>=x_drude1).and.(xM2(i)<= x_drude2) &
    )then
     FBx(i,j)=.false.
   else
     FBx(i,j)=.true.
...
do i=1,Nx
 do j=1,N_loc
  if( &
    (yM2(j)>=y_drude1).and.(yM2(j)<=y_drude2).and.(x(i)>=x_drude1).and.(x(i)<= x_drude2) &
    )then
     FBy(i,j)=.false.
   else
     FBy(i,j)=.true.
...
!   if(x(i) == x_detect .and. y(j) == y_detect)then
    H_sum = H_sum + abs((Hz(i,j) + Hz(i-1,j) + Hz(i,j-1) + Hz(i-1,j-1))/4.0)
!   endif
!  enddo
! enddo
endif
